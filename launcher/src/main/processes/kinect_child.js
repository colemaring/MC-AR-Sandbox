...
if (isPackaged) {
  try {
    ...
    console.error(
      `[kinect_child] Error adjusting module paths: ${pathError.message}`
    );
...
const main = async () => {
  if (kinect.open()) {
    ...
    kinect.on("rawDepthFrame", (depthFrame) => {
      ...
      for (let y = 0; y < height; y++) {
        ...
        for (let x = 0; x < width; x++) {
          ...
          // Read full 16-bit depth (little-endian)
          const depthInMM = depthFrame.readUInt16LE(byteIndex);
          row.push(depthInMM);
        }
        depthArray.push(row);
      }
